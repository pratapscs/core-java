...
{     
      ...
      * Entry contains K key, V value and 
      * Entry<K,V> next  (i.e. next entry on that location of bucket).
      * 
      * why entry class is static in hashmap?
      * A nested static class doesn't require an instance of the enclosing class. 
      * This means that you can create a HashMap.Entry by itself, without having 
      * to create a HashMap first (which you might not need at all). 1) Because 
      * it is not needed from the "outside": it is only used by the enclosing class
      */
